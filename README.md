...
The sync fetches the source catalog and GitHub previews without credentials, validates every 1200x600 PNG, then replaces `data/tools.json` and `assets/tools/` together only after all outputs succeed. Preview requests start at least 750 ms apart to stay below GitHub's observed rate window; HTTP 429 and transient 5xx responses retry with bounded waits.
